"""
Follower vehicle controller with simulated failure scenario.

This controller behaves like the standard follower vehicle controller, but it
adds a controlled failure condition after a predefined amount of time. This is
used to validate how the SPOT system reacts when the vehicle stops following
the drone during an active guidance mission.
"""
...
# Development flag used to enable debug information.
dev = True
...
# Maximum allowed time following the drone before simulating a failure.
...
print("Failure-scenario vehicle controller active. Waiting for navigation targets.")
...
while driver.step() != -1:
    ...
    while receiver.getQueueLength() > 0:
        ...
        try:
            ...
            if new_target_x != previous_target_x or new_target_y != previous_target_y:
                ...
                # Start failure timer when the first valid target is received.
                if follow_start_time is None:
                    follow_start_time = driver.getTime()
    ...
    # Once the car starts following the drone, measure how long it has been
    # active. After the configured threshold, simulate a vehicle failure.
    if follow_start_time is not None and not vehicle_failed:
        driving_time = driver.getTime() - follow_start_time

        if driving_time > TIEMPO_MAXIMO_SEGUIMIENTO:
            vehicle_failed = True
            print("\n" + "=" * 50)
            print(
                f"[car] Simulated vehicle failure triggered "
                f"after {TIEMPO_MAXIMO_SEGUIMIENTO:.1f} seconds."
            )
            print("[car] Vehicle stopped completely. Waiting for drone reaction.")
            print("=" * 50 + "\n")

    # If the vehicle has failed, force a full stop and skip all control logic.
    if vehicle_failed:
